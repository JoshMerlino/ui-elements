...
export default function InputField({ label, hint, className, height = "default", ...props }: Partial<InputFieldProps> & InputHTMLAttributes<HTMLInputElement>) {
	...
	const input = classNames("peer w-full h-full text-gray-700 dark:text-gray-200 font-roboto font-normal disabled:border-dashed transition-colors placeholder-shown:border placeholder-shown:border-gray-500/20 border px-3 py-2.5 focus:px-[11px] focus:!border-2 rounded-lg border-gray-400/40 dark:border-gray-400/25 focus:!border-primary backdrop-blur-2xl !bg-white/10 dark:!bg-gray-900/10 group-[.hascontents]:invalid:!border-error group-[.invalid]:!border-error placeholder:text-gray-600 dark:placeholder:text-gray-400 text-base", isTouchDevice ? "md:text-sm" : "text-sm");
