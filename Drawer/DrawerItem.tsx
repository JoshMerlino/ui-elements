...
export default function DrawerItem({ children, icon: Icon, active, waves = true, size = "default", ...props }: Partial<DrawerItemProps> & HTMLAttributes<HTMLDivElement>) {
	...
	const primary = "bg-primary/5 hover:bg-primary/10 text-primary-800 dark:text-primary hover:active:bg-primary/20";

	return (
		<div className={ classNames(base, active ? primary : gray, size === "dense" ? "h-12 gap-2" : "h-14 gap-4") }
			ref={ ref }
			{ ...props }>
